...
import {
  ...
  ExternalLink, Trash2, CalendarDays, FileText
} from "lucide-react";
...
// Category classifier. Hue is allowed here — on a calendar, position
// already encodes time, so colour is free to encode category. The six
// hues live as tokens at matched lightness so they read as one family
// instead of six competing -600s.
export function getCategoryStyle(ev: UnifiedCalEvent) {
  const cat = (ev.category || ev.type || "").toLowerCase();

  const make = (key: string, label: string, tag: string) => ({
    chip: `${key} cat-solid`,
    badge: `${key} cat-badge`,
    border: `${key} cat-block`,
    dot: `${key} cat-dot`,
    label,
    tag,
  });
  ...
  if (cat.includes("lomba") || cat.includes("competition") || cat.includes("challenge"))
    return make("cat-lomba", "Lomba / Competition", "Lomba");

  if (cat.includes("tugas") || cat.includes("homework") || cat.includes("school"))
    return make("cat-tugas", "Tugas Sekolah", "Tugas Sekolah");

  if (cat.includes("project") || cat.includes("projek") || cat.includes("academic"))
    return make("cat-projek", "Projek Akademik", "Projek");

  if (ev.type === "opens" || cat.includes("opens"))
    return make("cat-opens", "App Opens", "App Opens");

  if (ev.type === "deadline" || cat.includes("deadline") || cat.includes("application"))
    return make("cat-deadline", "Scholarship Apply", "Scholarship");

  return make("cat-agenda", "Agenda Harian", "Agenda");
}
    ...
    <div className="p-4 md:p-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl text-foreground">Calendar</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Today Button */}
          <Button variant="outline" size="sm" onClick={goToday} className="text-xs font-semibold px-3">
            Today
          </Button>
          ...
          <div className="flex items-center border border-border rounded-lg overflow-hidden bg-card shadow-xs">
            ...
            <span className="px-3.5 text-sm font-mono text-foreground min-w-[130px] text-center">
      ...
      {/* Legend. Data-driven off the same six tokens the events use, so it
          cannot drift out of sync — the old hand-written list had lost the
          "App Opens" category entirely. */}
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 panel px-4 py-2.5">
        <span className="eyebrow">Categories</span>
        {[
          ["cat-lomba", "Lomba"],
          ["cat-tugas", "Tugas Sekolah"],
          ["cat-projek", "Projek Akademik"],
          ["cat-opens", "App Opens"],
          ["cat-deadline", "Scholarship"],
          ["cat-agenda", "Agenda Harian"],
        ].map(([key, label]) => (
          <span key={key} className="flex items-center gap-1.5">
            <span className={`${key} cat-dot`} />
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-1.5">
                          <span
                            className={cn(
                              "inline-flex items-center justify-center text-xs font-semibold w-6 h-6 rounded-full transition-all",
                              isToday
                        ...
                        <div className="space-y-1 overflow-hidden flex-1">
                          {dayEvents.slice(0, 3).map((ev) => {
                            ...
                            return (
                              <div
                                ...
                                className={cn(
                                  "px-2 py-1 rounded text-xs font-semibold leading-tight truncate transition-all flex items-center justify-between gap-1 shadow-xs cursor-pointer active:scale-98",
                                  style.chip,
                              ...
                              >
                                ...
                                {ev.time && <span className="text-2xs font-mono shrink-0 opacity-90">{ev.time}</span>}
                              </div>
                            );
                          })}
                          ...
                          {dayEvents.length > 3 && (
                            <button
                              ...
                              className="text-2xs font-semibold text-primary hover:underline px-1 block"
                            >
                ...
                .map((dKey) => (
                  <div key={dKey} className="border-b border-border last:border-0 pb-4">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-semibold font-mono bg-muted text-foreground px-2 py-0.5 rounded">
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              <Badge variant="outline" className={cn("text-2xs shrink-0 font-semibold", style.badge)}>
                                {style.label}
                              </Badge>
          ...
          <DialogContent className="sm:max-w-md">
            <div className={cn("p-4 -mx-6 -mt-6 border-b rounded-t-lg", getCategoryStyle(activeEvent).border)}>
              <div className="flex items-center justify-between mb-1">
                <Badge variant="outline" className={cn("text-xs font-semibold", getCategoryStyle(activeEvent).badge)}>
                  {getCategoryStyle(activeEvent).label}
                </Badge>
                <span className="text-xs font-mono font-semibold text-muted-foreground">{activeEvent.date}</span>
              </div>
              <h2 className={cn("text-lg font-semibold text-foreground", activeEvent.completed && "line-through opacity-70")}>
                {activeEvent.title}
              </h2>
            </div>
              ...
              {activeEvent.type === "goal" && activeEvent.rawObject && (
                ...
                >
                  {activeEvent.completed ? (
                    ...
                    <>
                      <CheckCircle2 className="w-4 h-4 mr-1.5 text-ok" /> Mark Completed
                    </>
                  )}
                </Button>
              )}
